...
func NewEditCommand() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{Name: "validate, v", EnvVars: []string{"ETCDTOOL_VALIDATE"}, Usage: "Validate data before import"},
			&cli.StringFlag{Name: "format, f", Value: "JSON", EnvVars: []string{"ETCDTOOL_FORMAT"}, Usage: "Data serialization format YAML, TOML or JSON"},
			&cli.StringFlag{Name: "editor, e", Value: "vim", Usage: "Editor", EnvVars: []string{"EDITOR"}},
